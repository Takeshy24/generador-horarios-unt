...
export function AppSidebar({ role, userName }: AppSidebarProps) {
  ...
  return (
    ...
    >
      <aside
        className={cn(
          "fixed inset-y-0 left-0 z-30 flex h-screen flex-col overflow-hidden bg-[#111827] text-white transition-all duration-300 ease-in-out border-r border-slate-800",
          collapsed ? "w-20" : "w-64"
        )}
      >
        {/* Brand Logo Header */}
        ...
        {/* Navigation Items */}
        <nav className="flex-1 overflow-y-auto px-3 py-6 space-y-1.5 overflow-x-hidden">
          {items.map((item) => {
